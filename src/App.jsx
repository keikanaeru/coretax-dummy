...
const DEFAULT_CASES = {
  ...
  '3174082406050002': {
    ...
    deskripsiSoal:
      'Siti bekerja sebagai Staff Akuntansi (TK/0). Di e-Bupot tertera gajinya Rp80 juta, namun dia punya penghasilan sampingan dari luar usaha sebesar Rp40 juta dan satu bukti potong tambahan yang belum masuk otomatis. Mahasiswa wajib menambahkan penghasilan neto dan bukti potong manual tersebut di L-1.',
    ebupotGaji: 80000000,
    ebupotSampingan: 0,
    ebupotPtkpBase: 54000000,
    ebupotTanggungan: 0,
    ...
    kunciManualBupot: true,
    kunciManualBupotPPh: 500000,
    kunciStatusSPT: 'KURANG BAYAR',
  },
  '3174082406050003': {
...
function DataGridCard({
  ...
  const isCompactOnly = tightToolbar && !toolbar;
  ...
  return (
    <div className="bg-white border border-[#dfe5ee] rounded-sm overflow-hidden">
      ...
      {/*
        Coretax membedakan 2 jenis blok:
        1) Ikhtisar/form perhitungan: hanya toolbar kecil (refresh/copy/excel/pdf/filter) dan posisinya mepet header.
        2) Tabel daftar/detail: ada tombol Tambah, Hapus, Hapus Semua, Import Data lalu toolbar kecil.
      */}
      ...
      <div className="px-3 pb-3">
        {showTableActions && (
          <div
            className={`flex gap-2 ${
              isCompactOnly
                ? 'py-2 mb-0 border-b border-[#eef1f5] bg-white'
                : toolbar
                  ? 'pb-2 mb-0 bg-white'
                  : 'py-2 mb-0 border-b border-[#eef1f5] bg-white'
            }`}
        ...
        <div className="overflow-x-auto border-x border-b border-[#eef1f5]">
          {children}
        </div>
        {footerText && (
...
function AccordionCard({ title, subtitle = '', open = true, onToggle, children, badge = null }) {
  return (
    <div className="bg-white border border-[#dfe5ee] rounded-sm overflow-hidden">
      <button
        type="button"
        onClick={onToggle}
        className="w-full bg-[#17245c] text-white px-4 py-3 text-[12px] font-bold flex items-center justify-between gap-3"
      >
        <span className="flex items-center gap-2 text-left">
          <ChevronDown size={16} className={`transition ${open ? '' : '-rotate-90'}`} />
          <span>{title}</span>
          {badge && <span className="ml-2 rounded-full bg-[#ffd542] px-2 py-0.5 text-[10px] font-black text-[#17245c]">{badge}</span>}
        </span>
        {subtitle && <span className="text-[10px] font-semibold text-slate-200">{subtitle}</span>}
      </button>
...
function InlineNotice({ tone = 'info', children }) {
  ...
  return <div className={`rounded-lg border px-3 py-2 text-[11px] leading-relaxed ${styles[tone]}`}>{children}</div>;
}
...
function App() {
  ...
  const [bankSoalBadan, setBankSoalBadan] = useState(DEFAULT_COMPANY_CASES);
  const [loginRole, setLoginRole] = useState('mahasiswa');
  const [loginTaxpayerType, setLoginTaxpayerType] = useState(''); // 'op' atau 'badan' untuk landing login mahasiswa
  ...
  // ================= STATE FORM INPUT PENGAJAR - SPT BADAN =================
  const [teacherCaseTab, setTeacherCaseTab] = useState('op');
  const [newBadanNpwp, setNewBadanNpwp] = useState('');
  const [newBadanNama, setNewBadanNama] = useState('');
  const [newBadanDeskripsi, setNewBadanDeskripsi] = useState('');
  const [newBadanJenisUsaha, setNewBadanJenisUsaha] = useState('Perdagangan');
  const [newBadanKlu, setNewBadanKlu] = useState('46900 - Perdagangan Besar Berbagai Macam Barang');
  const [newBadanPendapatan, setNewBadanPendapatan] = useState(0);
  const [newBadanHpp, setNewBadanHpp] = useState(0);
  const [newBadanBebanUsaha, setNewBadanBebanUsaha] = useState(0);
  const [newBadanPendapatanLain, setNewBadanPendapatanLain] = useState(0);
  const [newBadanBebanLain, setNewBadanBebanLain] = useState(0);
  const [newBadanKoreksiPositif, setNewBadanKoreksiPositif] = useState(0);
  const [newBadanKoreksiNegatif, setNewBadanKoreksiNegatif] = useState(0);
  const [newBadanKompensasiRugi, setNewBadanKompensasiRugi] = useState(0);
  const [newBadanPPh22, setNewBadanPPh22] = useState(0);
  const [newBadanPPh23, setNewBadanPPh23] = useState(0);
  const [newBadanPPh25, setNewBadanPPh25] = useState(0);
  const [newBadanAsetHarga, setNewBadanAsetHarga] = useState(0);
  const [newBadanAsetAkumulasi, setNewBadanAsetAkumulasi] = useState(0);
  const [newBadanPemegangSaham, setNewBadanPemegangSaham] = useState('Pemegang Saham Utama');
  const [newBadanSahamPersen, setNewBadanSahamPersen] = useState(100);
  const [newBadanKunciKoreksiPositif, setNewBadanKunciKoreksiPositif] = useState(0);
  const [newBadanKunciStatus, setNewBadanKunciStatus] = useState('KURANG BAYAR');
  ...
  const [openOpSections, setOpenOpSections] = useState({ A: true, B: false, C: false, D: false, E: false });
  const [openHartaSections, setOpenHartaSections] = useState({ kas: true, piutang: false, investasi: false, bergerak: false, tidakBergerak: false, lainnya: false, ikhtisar: false });
  const [manualUtangList, setManualUtangList] = useState([]);
  const [manualKeluargaList, setManualKeluargaList] = useState([]);
  const [manualPekerjaanList, setManualPekerjaanList] = useState([]);
  const [manualBupotList, setManualBupotList] = useState([]);
  const [spouseNik, setSpouseNik] = useState('');
  const [spouseName, setSpouseName] = useState('');
  const [spouseMergeSubmitted, setSpouseMergeSubmitted] = useState(false);
  ...
  const resetStudentForm = () => {
    ...
    setOpenOpSections({ A: true, B: false, C: false, D: false, E: false });
    setOpenHartaSections({ kas: true, piutang: false, investasi: false, bergerak: false, tidakBergerak: false, lainnya: false, ikhtisar: false });
    setManualUtangList([]);
    setManualKeluargaList([]);
    setManualPekerjaanList([]);
    setManualBupotList([]);
    setSpouseNik('');
    setSpouseName('');
    setSpouseMergeSubmitted(false);
  ...
  const handleLogin = (e) => {
    ...
    if (!loginTaxpayerType) {
      setErrorMsg('Pilih dulu jenis praktikum: SPT Orang Pribadi atau SPT Badan.');
      return;
    }

    if (loginTaxpayerType === 'badan') {
      if (!activeBadanNpwp || !bankSoalBadan[activeBadanNpwp]) {
        setErrorMsg('NPWP Badan tidak terdaftar di bank kasus praktikum.');
        return;
      }
      resetStudentForm();
      setSelectedJenisPajak('SPT Tahunan PPh Wajib Pajak Badan');
      setSelectedPeriode('Januari - Desember 2025');
      setSelectedModelSpt('Normal');
      setIsLoggedIn(true);
    ...
    resetStudentForm();
    setSelectedJenisPajak('SPT Tahunan PPh Wajib Pajak Orang Pribadi');
    setSelectedPeriode('Januari - Desember 2025');
    setSelectedModelSpt('Normal');
    setIsLoggedIn(true);
    setCurrentScreen('dashboard');
  ...
  const handleCreateCompanyCase = (e) => {
    e.preventDefault();

    if (!newBadanNpwp || !newBadanNama) {
      alert('NPWP dan nama badan harus diisi.');
      return;
    }

    const customCompanyCase = {
      npwp: newBadanNpwp,
      namaBadan: newBadanNama,
      shortName: newBadanNama.toUpperCase(),
      email: `tax@${newBadanNama.toLowerCase().replace(/[^a-z0-9]+/g, '') || 'company'}.co.id`,
      telepon: '021-0000-0000',
      alamat: 'Alamat WP Badan disesuaikan oleh pengajar',
      klu: newBadanKlu || '46900 - Perdagangan Besar Berbagai Macam Barang',
      jenisUsaha: newBadanJenisUsaha || 'Perdagangan',
      namaKasus: 'Kasus Custom SPT Badan',
      deskripsiSoal: newBadanDeskripsi || 'Kasus custom SPT Badan dari pengajar.',
      pendapatan: Number(newBadanPendapatan),
      hpp: Number(newBadanHpp),
      bebanUsaha: Number(newBadanBebanUsaha),
      pendapatanLain: Number(newBadanPendapatanLain),
      bebanLain: Number(newBadanBebanLain),
      koreksiPositif: Number(newBadanKoreksiPositif),
      koreksiNegatif: Number(newBadanKoreksiNegatif),
      kompensasiRugi: Number(newBadanKompensasiRugi),
      kreditPPh22: Number(newBadanPPh22),
      kreditPPh23: Number(newBadanPPh23),
      kreditPPh25: Number(newBadanPPh25),
      asetHarga: Number(newBadanAsetHarga),
      asetAkumulasi: Number(newBadanAsetAkumulasi),
      pemegangSaham: newBadanPemegangSaham || 'Pemegang Saham Utama',
      sahamPersen: Number(newBadanSahamPersen),
      kunciKoreksiPositif: Number(newBadanKunciKoreksiPositif),
      kunciStatusSPT: newBadanKunciStatus || 'KURANG BAYAR',
    };

    setBankSoalBadan({
      ...bankSoalBadan,
      [newBadanNpwp]: customCompanyCase,
    });

    setNewBadanNpwp('');
    setNewBadanNama('');
    setNewBadanDeskripsi('');
    setNewBadanPendapatan(0);
    setNewBadanHpp(0);
    setNewBadanBebanUsaha(0);
    setNewBadanPendapatanLain(0);
    setNewBadanBebanLain(0);
    setNewBadanKoreksiPositif(0);
    setNewBadanKoreksiNegatif(0);
    setNewBadanKompensasiRugi(0);
    setNewBadanPPh22(0);
    setNewBadanPPh23(0);
    setNewBadanPPh25(0);
    setNewBadanAsetHarga(0);
    setNewBadanAsetAkumulasi(0);
    setNewBadanPemegangSaham('Pemegang Saham Utama');
    setNewBadanSahamPersen(100);
    setNewBadanKunciKoreksiPositif(0);
    alert('Soal SPT Badan berhasil disimpan ke bank kasus praktikum. NPWP ini bisa dipakai untuk login SPT Badan.');
  };

  const handleDeleteCompanyCase = (id) => {
    const updated = { ...bankSoalBadan };
    delete updated[id];
    setBankSoalBadan(updated);
    if (activeBadanNpwp === id) {
      const next = Object.keys(updated)[0] || '';
      setActiveBadanNpwp(next);
    }
  };


  const toggleOpSection = (key) => {
    setOpenOpSections((prev) => ({ ...prev, [key]: !prev[key] }));
  };

  const toggleHartaSection = (key) => {
    setOpenHartaSections((prev) => ({ ...prev, [key]: !prev[key] }));
  };

  const handleAddUtangManual = () => {
    const nextSaldo = manualUtangList.length === 0 ? 5000000 : 2500000;
    setManualUtangList((prev) => [
      ...prev,
      { kreditur: `Kreditur Manual ${prev.length + 1}`, tahun: '2025', saldo: nextSaldo, keterangan: 'Diinput manual' },
    ]);
    setAnswerUtang('Ya');
    setFormUtang((prev) => onlyNumber(prev) + nextSaldo);
  };

  const handleAddKeluargaManual = (type = 'Anak') => {
    const isSpouse = type === 'Istri/Suami';
    setManualKeluargaList((prev) => [
      ...prev,
      {
        nama: isSpouse ? spouseName || 'Pasangan Wajib Pajak' : `Tanggungan Manual ${prev.length + 1}`,
        nik: isSpouse ? spouseNik || '-' : '-',
        hubungan: type,
        pekerjaan: isSpouse ? 'Mengurus Rumah Tangga' : 'Pelajar',
      },
    ]);
    if (!isSpouse) setFormTanggungan((prev) => clampTanggungan(onlyNumber(prev) + 1));
    if (isSpouse && !String(formStatusPerkawinan || '').startsWith('K/')) setFormStatusPerkawinan('K/0');
  };

  const handleAddPekerjaanManual = () => {
    const bruto = manualPekerjaanList.length === 0 ? 12000000 : 8000000;
    setManualPekerjaanList((prev) => [
      ...prev,
      { pemberiKerja: `Pemberi Kerja Manual ${prev.length + 1}`, npwp: '09.999.999.9-999.000', bruto, pengurang: 0, neto: bruto },
    ]);
    setAnswerPekerjaan('Ya');
    setFormGaji((prev) => onlyNumber(prev) + bruto);
  };

  const handleAddBupotManual = () => {
    const pph = activeCase?.kunciManualBupotPPh || 500000;
    const bruto = manualBupotList.length === 0 ? 10000000 : 5000000;
    setManualBupotList((prev) => [
      ...prev,
      { pemotong: `Pemotong Manual ${prev.length + 1}`, npwp: '08.888.888.8-888.000', nomor: `BP-MANUAL-${String(prev.length + 1).padStart(3, '0')}`, tanggal: '31-12-2025', jenis: 'PPh 21', bruto, pph },
    ]);
    setAnswerPotongPihakLain('Ya');
    setFormKreditPajak((prev) => onlyNumber(prev) + pph);
  };

  const handleSubmitSpouseMerge = () => {
    if (!spouseNik || !spouseName) {
      setNotifSistem('Lengkapi NIK dan nama pasangan terlebih dahulu sebelum mengajukan penggabungan NPWP.');
      setTimeout(() => setNotifSistem(''), 4500);
      return;
    }
    setSpouseMergeSubmitted(true);
    setFormStatusPerkawinan((prev) => (String(prev || '').startsWith('K/') ? prev : 'K/0'));
    if (!manualKeluargaList.some((item) => item.hubungan === 'Istri/Suami')) handleAddKeluargaManual('Istri/Suami');
    setNotifSistem('Permohonan penggabungan NPWP pasangan berhasil dibuat. Data pasangan ikut terbaca di L-1 bagian keluarga.');
    setTimeout(() => setNotifSistem(''), 6000);
  };
  ...
  const validateBeforeSubmit = () => {
    ...
    const checkManualBupot = target.kunciManualBupot ? manualBupotList.length > 0 : true;

    return checkTanggungan && checkSampingan && checkStatus && checkManualBupot;
  };
  ...
  const currentTaxpayerName = isBadanSpt
    ? activeBadanCase?.shortName || '-'
    : activeCase?.shortName || '-';

  const currentTaxpayerId = isBadanSpt
    ? activeBadanCase?.npwp || activeBadanNpwp || '-'
    : activeCase?.npwp || activeNik || '-';

  const currentReturnType = isBadanSpt
    ? 'SPT Tahunan PPh Wajib Pajak Badan'
    : 'SPT Tahunan PPh Wajib Pajak Orang Pribadi';

  const currentTaxDue = isBadanSpt
    ? hasilBadan.pphTerutang
    : hasilPajak.pajakTerutang;

  const currentTaxCredit = isBadanSpt
    ? hasilBadan.totalKredit
    : onlyNumber(formKreditPajak);

  const getReviewRows = () => {
    if (isBadanSpt) {
      return [
        ['Jenis SPT', currentReturnType],
        ['NPWP Badan', currentTaxpayerId],
        ['Nama Wajib Pajak', currentTaxpayerName],
        ['Periode Pajak', selectedPeriode || 'Januari - Desember 2025'],
        ['Model SPT', selectedModelSpt || 'Normal'],
        ['Laba Komersial', formatRupiah(hasilBadan.labaKomersial)],
        ['Koreksi Fiskal Positif', formatRupiah(badanKoreksiPositif)],
        ['Koreksi Fiskal Negatif', formatRupiah(badanKoreksiNegatif)],
        ['Penghasilan Kena Pajak', formatRupiah(hasilBadan.pkp)],
        ['PPh Badan Terutang', formatRupiah(hasilBadan.pphTerutang)],
        ['Total Kredit Pajak', formatRupiah(hasilBadan.totalKredit)],
        ['Status Akhir', `${hasilBadan.status} - ${formatRupiah(hasilBadan.nominal)}`],
      ];
    }

    return [
      ['Jenis SPT', currentReturnType],
      ['NIK/NPWP', currentTaxpayerId],
      ['Nama Wajib Pajak', currentTaxpayerName],
      ['Periode Pajak', selectedPeriode || 'Januari - Desember 2025'],
      ['Model SPT', selectedModelSpt || 'Normal'],
      ['Total Penghasilan Neto', formatRupiah(hasilPajak.totalNetto)],
      ['PTKP', formatRupiah(hasilPajak.totalPtkp)],
      ['Penghasilan Kena Pajak', formatRupiah(hasilPajak.pkp)],
      ['PPh Terutang', formatRupiah(hasilPajak.pajakTerutang)],
      ['Kredit Pajak', formatRupiah(formKreditPajak)],
      ['Status Akhir', `${hasilPajak.status} - ${formatRupiah(hasilPajak.nominal)}`],
    ];
  };

  const getValidationChecklist = () => {
    const base = [
      { label: 'Konsep SPT sudah dibuat', done: currentScreen === 'form-spt' || returnLifecycle !== 'DRAFT' },
      { label: 'Data sudah diposting dari sistem', done: returnLifecycle !== 'DRAFT' },
      { label: 'Status pajak sudah dihitung otomatis', done: true },
    ];

    if (isBadanSpt) {
      const koreksiSesuai = onlyNumber(badanKoreksiPositif) === onlyNumber(activeBadanCase?.kunciKoreksiPositif || 0);
      return [
        ...base,
        { label: 'Data laba rugi komersial sudah tersedia', done: onlyNumber(badanPendapatan) >= 0 },
        { label: 'Koreksi fiskal positif sesuai skenario kasus', done: koreksiSesuai },
        { label: 'Kredit pajak dan PPh Badan sudah direkonsiliasi', done: true },
      ];
    }

    return [
      ...base,
      { label: 'PTKP/tanggungan sudah diperiksa', done: clampTanggungan(formTanggungan) === clampTanggungan(activeCase?.kunciTanggungan || 0) },
      { label: 'Penghasilan sampingan sesuai skenario kasus', done: onlyNumber(formSampingan) === onlyNumber(activeCase?.kunciSampingan || 0) },
      { label: 'Bukti potong manual sudah ditambahkan jika diminta kasus', done: activeCase?.kunciManualBupot ? manualBupotList.length > 0 : true },
      { label: 'Kredit pajak sudah direkonsiliasi', done: true },
    ];
  };

  const buildRingkasanText = () => {
    const rows = getReviewRows();
    const checklist = getValidationChecklist();
    return [
      'RINGKASAN SPT - SIMULASI CORETAX',
      '=================================',
      `Tanggal Unduh : ${new Date().toLocaleString('id-ID')}`,
      `Status Sistem : ${lifecycleMeta.label}`,
      '',
      ...rows.map(([label, value]) => `${label}: ${value}`),
      '',
      'CHECKLIST VALIDASI',
      '------------------',
      ...checklist.map((item) => `${item.done ? '[OK]' : '[PERLU CEK]'} ${item.label}`),
      '',
      'Catatan: dokumen ini adalah ringkasan simulasi praktikum, bukan dokumen resmi DJP.',
    ].join('\n');
  };

  const buildBpeText = () => {
    const kode = bpeCode || makeBpeCode();
    return [
      'BUKTI PENERIMAAN ELEKTRONIK (BPE)',
      'SIMULASI CORETAX PRAKTIKUM',
      '================================',
      `Kode BPE       : ${kode}`,
      `Tanggal Lapor  : ${new Date().toLocaleString('id-ID')}`,
      `Jenis SPT      : ${currentReturnType}`,
      `Masa/Tahun     : ${selectedPeriode || 'Januari - Desember 2025'}`,
      `NPWP/NIK       : ${currentTaxpayerId}`,
      `Nama WP        : ${currentTaxpayerName}`,
      `Status SPT     : ${currentTaxResult.status}`,
      `Nominal        : ${formatRupiah(currentTaxResult.nominal)}`,
      '',
      'Dokumen ini dihasilkan oleh aplikasi simulasi pembelajaran dan bukan BPE resmi DJP.',
    ].join('\n');
  };
  ...
  const handleDownloadRingkasan = () => {
    const safeId = String(currentTaxpayerId || 'WP').replace(/\D/g, '') || 'WP';
    downloadTextFile(`Ringkasan-SPT-${isBadanSpt ? 'Badan' : 'OP'}-${safeId}.txt`, buildRingkasanText());
  };

  const handleDownloadBPE = () => {
    const safeId = String(currentTaxpayerId || 'WP').replace(/\D/g, '') || 'WP';
    downloadTextFile(`BPE-${isBadanSpt ? 'Badan' : 'OP'}-${safeId}.txt`, buildBpeText());
  };
            ...
            <form onSubmit={handleLogin} className="mt-7 space-y-5">
              ...
              {loginRole === 'mahasiswa' ? (
                <div className="space-y-5">
                  <div>
                    <div className="text-[12px] font-bold text-slate-600 mb-2">Pilih jenis praktikum</div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <button
                        type="button"
                        onClick={() => {
                          setLoginTaxpayerType('op');
                          setSelectedJenisPajak('SPT Tahunan PPh Wajib Pajak Orang Pribadi');
                          setErrorMsg('');
                        }}
                        className={`text-left rounded-2xl border-2 p-4 transition ${
                          loginTaxpayerType === 'op'
                            ? 'border-[#17245c] bg-[#17245c]/5 shadow-sm'
                            : 'border-slate-200 hover:border-[#17245c]/40'
                        }`}
                      >
                        <div className="flex items-center gap-2 font-black text-[#17245c]"><User size={18} /> SPT Orang Pribadi</div>
                        <div className="mt-2 text-[11px] text-slate-500 leading-relaxed">Masuk sebagai WP OP/karyawan untuk latihan PTKP, penghasilan, harta, utang, dan bukti potong.</div>
                      </button>
                      <button
                        type="button"
                        onClick={() => {
                          setLoginTaxpayerType('badan');
                          setSelectedJenisPajak('SPT Tahunan PPh Wajib Pajak Badan');
                          setErrorMsg('');
                        }}
                        className={`text-left rounded-2xl border-2 p-4 transition ${
                          loginTaxpayerType === 'badan'
                            ? 'border-[#17245c] bg-[#17245c]/5 shadow-sm'
                            : 'border-slate-200 hover:border-[#17245c]/40'
                        }`}
                      >
                        <div className="flex items-center gap-2 font-black text-[#17245c]"><Landmark size={18} /> SPT Badan</div>
                        <div className="mt-2 text-[11px] text-slate-500 leading-relaxed">Masuk sebagai WP Badan untuk latihan laporan keuangan, koreksi fiskal, kredit pajak, dan pemegang saham.</div>
                      </button>
                    </div>
                  </div>
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <select
                          ...
                          onChange={(e) => setActiveBadanNpwp(e.target.value)}
                        >
                          {Object.values(bankSoalBadan).map((item) => (
                            <option key={item.npwp} value={item.npwp}>{item.npwp} - {item.namaBadan}</option>
                          ))}
                        </select>
                      </div>
                      <div className="mt-3 rounded-xl bg-blue-50 border border-blue-100 p-3 text-[11px] text-blue-900 leading-relaxed">
                        Login Badan langsung mengarahkan wizard ke SPT Tahunan PPh Wajib Pajak Badan. Data laporan keuangan akan muncul setelah klik Posting SPT.
                      </div>
                    </div>
                  ) : (
                    <div>
                      <label className="block text-[12px] font-bold text-slate-600 mb-2">
                        Masukkan NIK Kasus OP
                      </label>
                      <div className="relative">
              ...
              >
                {loginRole === 'pengajar' ? 'MASUK PANEL PENGAJAR' : loginTaxpayerType === 'badan' ? 'MASUK SPT BADAN' : 'MASUK SPT ORANG PRIBADI'}
              </button>
            </form>
          </div>
        </div>
    ...
    return (
      <div className="min-h-screen bg-[#f3f6fb] font-sans">
        ...
        <main className="p-6 max-w-7xl mx-auto space-y-5">
          <div className="bg-white rounded-xl border shadow-sm p-2 flex flex-wrap gap-2">
            <button
              type="button"
              onClick={() => setTeacherCaseTab('op')}
              className={`h-10 px-4 rounded-lg text-[12px] font-black flex items-center gap-2 ${teacherCaseTab === 'op' ? 'bg-[#17245c] text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
            >
              <User size={15} /> Bank Soal SPT OP
            </button>
            <button
              type="button"
              onClick={() => setTeacherCaseTab('badan')}
              className={`h-10 px-4 rounded-lg text-[12px] font-black flex items-center gap-2 ${teacherCaseTab === 'badan' ? 'bg-[#17245c] text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
            >
              <Landmark size={15} /> Bank Soal SPT Badan
            </button>
          </div>
          ...
          {teacherCaseTab === 'op' && (
            <div className="grid grid-cols-1 lg:grid-cols-[420px_1fr] gap-6">
              <section className="bg-white rounded-xl border shadow-sm overflow-hidden">
                <SectionHeader>
                  <Layers size={16} />
                  Kustomisasi e-Bupot OP & Kunci Jawaban
                </SectionHeader>

                <form onSubmit={handleCreateCase} className="p-5 space-y-4">
                  <FieldInput label="1. NIK Dummy Target" value={newNik} onChange={setNewNik} placeholder="Masukkan 16 digit NIK" />
                  <FieldInput label="2. Nama Wajib Pajak" value={newNama} onChange={setNewNama} placeholder="Misal: Andi Wijaya" />

                  <div className="space-y-1">
                    <label className="block text-[11px] font-semibold text-slate-500">3. Narasi Deskripsi Masalah / Soal</label>
                    <textarea rows={4} value={newDeskripsi} onChange={(e) => setNewDeskripsi(e.target.value)} placeholder="Tulis instruksi pengerjaan untuk mahasiswa..." className="w-full rounded border px-3 py-2 text-[12px] outline-none focus:ring-1 focus:ring-blue-900" />
                  </div>

                  <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 space-y-3">
                    <div className="font-black text-amber-900 text-[12px]">Data e-Bupot OP Bawaan</div>
                  ...
                  <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4 space-y-3">
                    <div className="font-black text-emerald-900 text-[12px]">Kunci Jawaban Validasi OP</div>
                  ...
                  <button type="submit" className="w-full h-10 rounded-lg bg-[#17245c] text-white text-[12px] font-black hover:bg-[#0f1a45]">PUBLISH SOAL OP KE LAB</button>
                </form>
              </section>
              ...
              <section className="bg-white rounded-xl border shadow-sm overflow-hidden">
                <SectionHeader>
                  <BookOpen size={16} />
                  Daftar Bank Soal SPT Orang Pribadi
                </SectionHeader>

                <div className="p-5 space-y-3">
                  {Object.values(bankSoal).map((item) => (
                    <div key={item.nik} className="rounded-xl border bg-slate-50 p-4 flex flex-col xl:flex-row xl:items-center justify-between gap-4">
                      <div className="space-y-2">
                        <div className="flex flex-wrap items-center gap-2">
                          <span className="font-mono text-[12px] font-bold rounded bg-blue-50 border border-blue-100 px-2 py-1 text-blue-800">{item.nik}</span>
                          <span className="font-bold text-slate-800 text-[13px]">{item.namaWP} - {item.namaKasus}</span>
                        </div>
                        <p className="text-[12px] text-slate-500 leading-relaxed max-w-3xl">{item.deskripsiSoal}</p>
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-2 text-[11px] font-mono">
                          <div className="rounded bg-white border p-2">Gaji: <b>{formatRupiah(item.ebupotGaji)}</b></div>
                          <div className="rounded bg-white border p-2">Sampingan awal: <b>{formatRupiah(item.ebupotSampingan)}</b></div>
                          <div className="rounded bg-white border p-2">Tanggungan e-Bupot: <b className="text-rose-600">{item.ebupotTanggungan}</b></div>
                          <div className="rounded bg-white border p-2">Kunci: <b className="text-emerald-700">{item.kunciTanggungan}</b> / <b className="text-emerald-700">{formatRupiah(item.kunciSampingan || 0)}</b></div>
                        </div>
                      </div>

                      <button onClick={() => handleDeleteCase(item.nik)} className="h-9 w-9 rounded-lg border border-rose-200 bg-white text-rose-600 hover:bg-rose-50 flex items-center justify-center"><Trash2 size={16} /></button>
                    </div>
                  ))}
                </div>
              </section>
          ...
          {teacherCaseTab === 'badan' && (
            <div className="grid grid-cols-1 lg:grid-cols-[480px_1fr] gap-6">
              <section className="bg-white rounded-xl border shadow-sm overflow-hidden">
                <SectionHeader>
                  <Landmark size={16} />
                  Kustomisasi SPT Badan & Data Posting
                </SectionHeader>

                <form onSubmit={handleCreateCompanyCase} className="p-5 space-y-4">
                  <div className="rounded-xl border border-blue-100 bg-blue-50 p-3 text-[12px] text-blue-900 leading-relaxed">
                    Bagian ini yang mengatur kasus SPT Badan. Data yang diisi di sini akan muncul saat mahasiswa login pakai NPWP Badan lalu klik <b>Posting SPT</b>.
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <FieldInput label="1. NPWP Badan Dummy" value={newBadanNpwp} onChange={setNewBadanNpwp} placeholder="Contoh: 012345678901000" />
                    <FieldInput label="2. Nama WP Badan" value={newBadanNama} onChange={setNewBadanNama} placeholder="Misal: PT Contoh Jaya" />
                    <FieldInput label="Jenis Usaha" value={newBadanJenisUsaha} onChange={setNewBadanJenisUsaha} />
                    <FieldInput label="KLU" value={newBadanKlu} onChange={setNewBadanKlu} />
                  </div>

                  <div className="space-y-1">
                    <label className="block text-[11px] font-semibold text-slate-500">Narasi Deskripsi Masalah / Soal Badan</label>
                    <textarea rows={4} value={newBadanDeskripsi} onChange={(e) => setNewBadanDeskripsi(e.target.value)} placeholder="Contoh: WP Badan memiliki koreksi fiskal positif yang belum dimasukkan..." className="w-full rounded border px-3 py-2 text-[12px] outline-none focus:ring-1 focus:ring-blue-900" />
                  </div>

                  <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 space-y-3">
                    <div className="font-black text-amber-900 text-[12px]">Data Posting Laporan Komersial</div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      <FieldInput label="Pendapatan Usaha" type="number" value={newBadanPendapatan} onChange={setNewBadanPendapatan} />
                      <FieldInput label="HPP" type="number" value={newBadanHpp} onChange={setNewBadanHpp} />
                      <FieldInput label="Beban Usaha" type="number" value={newBadanBebanUsaha} onChange={setNewBadanBebanUsaha} />
                      <FieldInput label="Pendapatan Lain-lain" type="number" value={newBadanPendapatanLain} onChange={setNewBadanPendapatanLain} />
                      <FieldInput label="Beban Lain-lain" type="number" value={newBadanBebanLain} onChange={setNewBadanBebanLain} />
                    </div>
                  </div>

                  <div className="rounded-xl border border-indigo-200 bg-indigo-50 p-4 space-y-3">
                    <div className="font-black text-indigo-900 text-[12px]">Data Rekonsiliasi Fiskal & Kredit Pajak</div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      <FieldInput label="Koreksi Positif Awal" type="number" value={newBadanKoreksiPositif} onChange={setNewBadanKoreksiPositif} />
                      <FieldInput label="Koreksi Negatif" type="number" value={newBadanKoreksiNegatif} onChange={setNewBadanKoreksiNegatif} />
                      <FieldInput label="Kompensasi Rugi Fiskal" type="number" value={newBadanKompensasiRugi} onChange={setNewBadanKompensasiRugi} />
                      <FieldInput label="PPh 22" type="number" value={newBadanPPh22} onChange={setNewBadanPPh22} />
                      <FieldInput label="PPh 23" type="number" value={newBadanPPh23} onChange={setNewBadanPPh23} />
                      <FieldInput label="PPh 25" type="number" value={newBadanPPh25} onChange={setNewBadanPPh25} />
                    </div>
                  </div>

                  <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 space-y-3">
                    <div className="font-black text-slate-800 text-[12px]">Data Penyusutan & Pemegang Saham</div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      <FieldInput label="Harga Perolehan Aset" type="number" value={newBadanAsetHarga} onChange={setNewBadanAsetHarga} />
                      <FieldInput label="Akumulasi Penyusutan" type="number" value={newBadanAsetAkumulasi} onChange={setNewBadanAsetAkumulasi} />
                      <FieldInput label="Pemegang Saham" value={newBadanPemegangSaham} onChange={setNewBadanPemegangSaham} />
                      <FieldInput label="% Saham" type="number" value={newBadanSahamPersen} onChange={setNewBadanSahamPersen} />
                    </div>
                  </div>

                  <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4 space-y-3">
                    <div className="font-black text-emerald-900 text-[12px]">Kunci Validasi SPT Badan</div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      <FieldInput label="Kunci Koreksi Positif" type="number" value={newBadanKunciKoreksiPositif} onChange={setNewBadanKunciKoreksiPositif} />
                      <SelectBox
                        label="Kunci Status SPT"
                        value={newBadanKunciStatus}
                        onChange={setNewBadanKunciStatus}
                        options={[{ value: 'NIHIL', label: 'NIHIL' }, { value: 'KURANG BAYAR', label: 'KURANG BAYAR' }, { value: 'LEBIH BAYAR', label: 'LEBIH BAYAR' }]}
                      />
                    </div>
                  </div>

                  <button type="submit" className="w-full h-10 rounded-lg bg-[#17245c] text-white text-[12px] font-black hover:bg-[#0f1a45]">PUBLISH SOAL BADAN KE LAB</button>
                </form>
              </section>
              ...
              <section className="bg-white rounded-xl border shadow-sm overflow-hidden">
                <SectionHeader>
                  <BookOpen size={16} />
                  Daftar Bank Soal SPT Badan
                </SectionHeader>

                <div className="p-5 space-y-3">
                  {Object.values(bankSoalBadan).map((item) => {
                    const labaKomersialItem = Number(item.pendapatan || 0) - Number(item.hpp || 0) - Number(item.bebanUsaha || 0) + Number(item.pendapatanLain || 0) - Number(item.bebanLain || 0);
                    const labaFiskalItem = labaKomersialItem + Number(item.koreksiPositif || 0) - Number(item.koreksiNegatif || 0) - Number(item.kompensasiRugi || 0);
                    const pphItem = Math.max(0, Math.floor(labaFiskalItem / 1000) * 1000) * 0.22;
                    const kreditItem = Number(item.kreditPPh22 || 0) + Number(item.kreditPPh23 || 0) + Number(item.kreditPPh25 || 0);
                    const statusItem = pphItem - kreditItem > 0 ? 'KURANG BAYAR' : pphItem - kreditItem < 0 ? 'LEBIH BAYAR' : 'NIHIL';

                    return (
                      <div key={item.npwp} className="rounded-xl border bg-slate-50 p-4 flex flex-col xl:flex-row xl:items-center justify-between gap-4">
                        <div className="space-y-2">
                          <div className="flex flex-wrap items-center gap-2">
                            <span className="font-mono text-[12px] font-bold rounded bg-blue-50 border border-blue-100 px-2 py-1 text-blue-800">{item.npwp}</span>
                            <span className="font-bold text-slate-800 text-[13px]">{item.namaBadan} - {item.namaKasus}</span>
                          </div>
                          <p className="text-[12px] text-slate-500 leading-relaxed max-w-3xl">{item.deskripsiSoal}</p>
                          <div className="grid grid-cols-1 md:grid-cols-4 gap-2 text-[11px] font-mono">
                            <div className="rounded bg-white border p-2">Pendapatan: <b>{formatRupiah(item.pendapatan)}</b></div>
                            <div className="rounded bg-white border p-2">Koreksi awal: <b>{formatRupiah(item.koreksiPositif)}</b></div>
                            <div className="rounded bg-white border p-2">Kunci koreksi: <b className="text-emerald-700">{formatRupiah(item.kunciKoreksiPositif)}</b></div>
                            <div className="rounded bg-white border p-2">Estimasi status: <b className="text-[#17245c]">{statusItem}</b></div>
                          </div>
                        </div>

                        <button onClick={() => handleDeleteCompanyCase(item.npwp)} className="h-9 w-9 rounded-lg border border-rose-200 bg-white text-rose-600 hover:bg-rose-50 flex items-center justify-center"><Trash2 size={16} /></button>
                      </div>
                    );
                  })}
                </div>
    ...
    <div className="min-h-screen bg-[#eef2f7] font-sans text-slate-700 text-[12px]">
      ...
      <header className="bg-white shadow-sm border-b sticky top-0 z-40">
        ...
        <nav className="px-6 py-2 flex items-center gap-2 overflow-x-auto text-[#17245c] font-semibold">
          {[
            ...
            const activeNav = item === 'SPT' ? currentScreen !== 'portal-saya' : item === 'Portal Saya' && currentScreen === 'portal-saya';
            return (
            <button
              key={item}
              onClick={() => {
                if (item === 'Portal Saya') setCurrentScreen('portal-saya');
                if (item === 'SPT') setCurrentScreen('dashboard');
              }}
      ...
      <div className="flex">
        ...
        <aside className="hidden lg:block w-[280px] bg-white border-r min-h-[calc(100vh-118px)] sticky top-[118px]">
          <div className="p-4">
            ...
            <div className="rounded-xl bg-[#17245c] text-white p-4 mb-4">
              ...
              <div className="mt-2 font-semibold uppercase">{displayTaxpayer?.shortName}</div>
            ...
            <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-4">
              ...
              <div className="mt-2 text-[11px] leading-relaxed text-slate-700">
                {isBadanSpt ? activeBadanCase?.deskripsiSoal : activeCase?.deskripsiSoal}
              </div>
            </div>
          </div>
        </aside>
        ...
        <main className="flex-1 p-5 xl:p-6 overflow-x-hidden">
          ...
          {/* PORTAL SAYA - PROFIL & PENGGABUNGAN NPWP PASUTRI */}
          {currentScreen === 'portal-saya' && (
            <div className="max-w-6xl mx-auto space-y-5">
              ...
              <div className="bg-white rounded-xl border shadow-sm overflow-hidden">
                <div className="p-6 flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b">
                  <div>
                    <h1 className="text-[28px] text-[#17245c] font-normal">Portal Saya</h1>
                    <p className="mt-2 text-slate-500">Simulasi pengelolaan profil WP, data keluarga, dan permohonan penggabungan NPWP pasangan.</p>
                  </div>
                  <button onClick={() => setCurrentScreen('dashboard')} className="h-10 px-4 rounded bg-[#17245c] text-white font-bold flex items-center gap-2">
                    <FileText size={15} />
                    Kembali ke SPT
                  </button>
                </div>

                <div className="p-6 grid grid-cols-1 lg:grid-cols-[0.9fr_1.1fr] gap-5">
                  <div className="rounded-xl border bg-slate-50 p-5 space-y-3">
                    <div className="font-black text-[#17245c] text-[15px]">Profil Wajib Pajak</div>
                    <div className="grid grid-cols-1 gap-3">
                      <FieldInput label="NIK/NPWP" value={activeCase?.npwp || activeNik || '-'} readOnly />
                      <FieldInput label="Nama Wajib Pajak" value={activeCase?.shortName || '-'} readOnly />
                      <FieldInput label="Status Perkawinan Saat Ini" value={formStatusPerkawinan || activeCase?.statusPerkawinan || 'TK/0'} readOnly />
                    </div>
                    <InlineNotice tone="info">
                      Menu ini disiapkan untuk skenario WP baru menikah dan ingin menggabungkan NPWP pasangan. Setelah permohonan dibuat, data pasangan akan ikut muncul di L-1 bagian C.
                    </InlineNotice>
                  </div>

                  <div className="rounded-xl border overflow-hidden">
                    <div className="bg-[#17245c] text-white px-4 py-3 font-bold flex items-center gap-2"><User size={15} /> Permohonan Penggabungan NPWP Suami/Istri</div>
                    <div className="p-5 space-y-4">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <FieldInput label="NIK/NPWP Pasangan" value={spouseNik} onChange={setSpouseNik} placeholder="Masukkan NIK/NPWP pasangan" />
                        <FieldInput label="Nama Pasangan" value={spouseName} onChange={setSpouseName} placeholder="Masukkan nama pasangan" />
                        <SelectBox
                          label="Status Permohonan"
                          value={spouseMergeSubmitted ? 'Diajukan' : ''}
                          onChange={() => {}}
                          placeholder="Belum diajukan"
                          disabled
                          options={[{ value: 'Diajukan', label: 'Diajukan - Simulasi' }]}
                        />
                        <FieldInput label="Dampak ke Status SPT" value={spouseMergeSubmitted ? 'Status diarahkan ke K/0 dan pasangan masuk daftar keluarga' : 'Belum ada perubahan'} readOnly />
                      </div>
                      <div className="flex flex-wrap gap-2">
                        <button onClick={handleSubmitSpouseMerge} className="h-10 px-5 rounded bg-[#17245c] text-white font-bold flex items-center gap-2">
                          <Send size={15} />
                          Ajukan Penggabungan NPWP
                        </button>
                        <button onClick={() => { setSpouseNik(''); setSpouseName(''); setSpouseMergeSubmitted(false); }} className="h-10 px-5 rounded bg-white border text-slate-600 font-bold">
                          Reset Permohonan
                        </button>
                      </div>
                      {spouseMergeSubmitted && (
                        <InlineNotice tone="ok">
                          Permohonan simulasi berhasil dibuat. Cek L-1 bagian C untuk melihat data pasangan sebagai anggota keluarga.
                        </InlineNotice>
                      )}
                    </div>
                  </div>
            ...
            <div className="max-w-[1280px] mx-auto space-y-5">
              ...
              <div className="bg-white rounded-xl border shadow-sm overflow-hidden">
                ...
                <div className="px-6 pt-7">
                  <div className="flex border-b">
                    {(isBadanSpt ? ['Induk', 'L-1', 'L-2', 'L-3', 'L-4', 'L-5', 'Ringkasan'] : ['Induk', 'L-1']).map((tab) => {
                      const tabLabel = isBadanSpt
                        ? ({
                            Induk: 'Induk',
                            'L-1': 'Laba Rugi',
                            'L-2': 'Rekonsiliasi Fiskal',
                            'L-3': 'Kredit Pajak',
                            'L-4': 'Penyusutan',
                            'L-5': 'Pemegang Saham',
                            Ringkasan: 'Ringkasan',
                      ...
                      return (
                        <button
                          ...
                          className={`h-10 px-4 text-[14px] font-semibold border-b-2 whitespace-nowrap ${
                            activeTab === tab
                  ...
                  <div className="p-6 space-y-4">
                    <div className="rounded-xl border border-blue-100 bg-gradient-to-r from-blue-50 to-white p-4">
                      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                        <div>
                          <div className="text-[11px] uppercase tracking-widest font-black text-blue-700">Mode Praktikum SPT Badan</div>
                          <div className="mt-1 text-lg font-black text-[#17245c]">{activeBadanCase?.namaBadan}</div>
                          <div className="text-[12px] text-slate-500 max-w-2xl">Alur ini difokuskan ke pengisian SPT Badan versi kampus: identitas badan, laporan laba rugi, rekonsiliasi fiskal, kredit pajak, penyusutan, pemegang saham, dan ringkasan sebelum bayar/lapor.</div>
                        </div>
                        <div className="grid grid-cols-3 gap-2 text-center text-[10px] font-bold">
                          {['Induk', 'Laba Rugi', 'Fiskal', 'Kredit', 'Aset', 'Submit'].map((item, index) => (
                            <div key={item} className={`rounded-lg border px-3 py-2 ${index === 0 ? 'bg-[#17245c] text-white border-[#17245c]' : 'bg-white text-slate-500'}`}>
                ...
                {isBadanSpt && activeTab === 'L-1' && (
                  <div className="p-6 space-y-4">
                    <DataGridCard title="L-1 DATA KOMERSIAL / LAPORAN LABA RUGI" tightToolbar footerText={`LABA SEBELUM PAJAK KOMERSIAL ${hasilBadan.labaKomersial.toLocaleString('id-ID')}`}>
                      <div className="p-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 border-b bg-white">
                        ...
                        <div className="rounded-lg border bg-[#17245c] text-white p-3">
                          <div className="text-[10px] text-blue-100 uppercase tracking-widest">Laba Komersial</div>
                          <div className="mt-1 font-black text-[15px]">{formatRupiah(hasilBadan.labaKomersial)}</div>
                        </div>
                      </div>
                ...
                {isBadanSpt && activeTab === 'L-2' && (
                  <div className="p-6 space-y-4">
                    <DataGridCard title="L-2 REKONSILIASI FISKAL" tightToolbar footerText={`PENGHASILAN KENA PAJAK ${hasilBadan.pkp.toLocaleString('id-ID')}`}>
                    ...
                    <div className={`rounded-xl border p-4 text-[12px] ${onlyNumber(badanKoreksiPositif) === onlyNumber(activeBadanCase?.kunciKoreksiPositif || 0) ? 'border-emerald-200 bg-emerald-50 text-emerald-900' : 'border-amber-200 bg-amber-50 text-amber-900'}`}>
                      <div className="font-black mb-1">Status koreksi fiskal:</div>
                      {onlyNumber(badanKoreksiPositif) === onlyNumber(activeBadanCase?.kunciKoreksiPositif || 0)
                        ? 'Koreksi fiskal positif sudah sesuai kunci kasus. Lanjutkan ke kredit pajak.'
                        : `Periksa kembali koreksi fiskal positif. Target kasus ini adalah ${formatRupiah(activeBadanCase?.kunciKoreksiPositif || 0)}.`}
                    </div>
                ...
                {isBadanSpt && activeTab === 'L-3' && (
                  <div className="p-6 space-y-4">
                    <DataGridCard title="L-3 KREDIT PAJAK" tightToolbar footerText={`TOTAL KREDIT PAJAK ${hasilBadan.totalKredit.toLocaleString('id-ID')}`}>
                      <div className="p-4 grid grid-cols-1 md:grid-cols-3 gap-3 border-b bg-white">
                ...
                {!isBadanSpt && activeTab === 'L-1' && (
                  <div className="p-6 space-y-4">
                    <div className="rounded-xl border bg-slate-50 p-4">
                      <div className="font-black text-[#17245c] mb-2">Daftar Lampiran L-1</div>
                      <ul className="list-disc pl-6 text-[13px] text-slate-600 space-y-1">
                    ...
                    <AccordionCard
                      title="A. HARTA PADA AKHIR TAHUN PAJAK"
                      subtitle={`Total: ${formatRupiah(totalHarta)}`}
                      badge="accordion"
                      open={openOpSections.A}
                      onToggle={() => toggleOpSection('A')}
                    >
                      <div className="space-y-3 p-4 bg-white">
                        <InlineNotice tone="info">
                          Bagian A dibuat seperti Coretax: klik judul untuk buka/tutup, lalu isi detail harta di subbagian 1 sampai 7. Ikhtisar harta otomatis mengikuti nilai detail yang diinput.
                        </InlineNotice>

                        <AccordionCard title="1. KAS DAN SETARA KAS" open={openHartaSections.kas} onToggle={() => toggleHartaSection('kas')} subtitle={formatRupiah(formHartaKas)}>
                          <DataGridCard title="Daftar Kas dan Setara Kas" toolbar footerText={`JUMLAH TABEL 1 ${formHartaKas.toLocaleString('id-ID')}`}>
                            <div className="p-3 bg-white max-w-md border-b">
                              <FieldInput label="Nilai Kas/Tabungan" type="number" value={formHartaKas} onChange={setFormHartaKas} />
                            </div>
                            <YellowTable columns={['TINDAKAN', 'NO.', 'KODE', 'DESKRIPSI', 'NOMOR AKUN', 'ATAS NAMA', 'NAMA BANK/INSTITUSI', 'LOKASI HARTA', 'NILAI']}>
                              ...
                              {formHartaKas > 0 ? (
                                <tr>
                                  ...
                                  <td className="border p-2">{displayTaxpayer?.shortName}</td>
                        ...
                        <AccordionCard title="2. PIUTANG" open={openHartaSections.piutang} onToggle={() => toggleHartaSection('piutang')}>
                          <DataGridCard title="Daftar Piutang" toolbar footerText="JUMLAH TABEL 2 0">
                        ...
                        <AccordionCard title="3. INVESTASI/SEKURITAS" open={openHartaSections.investasi} onToggle={() => toggleHartaSection('investasi')} subtitle={formatRupiah(formHartaInvestasi)}>
                          <DataGridCard title="Daftar Investasi/Sekuritas" toolbar footerText={`JUMLAH TABEL 3 ${formHartaInvestasi.toLocaleString('id-ID')}`}>
                            <div className="p-3 bg-white max-w-md border-b"><FieldInput label="Nilai Investasi/Sekuritas" type="number" value={formHartaInvestasi} onChange={setFormHartaInvestasi} /></div>
                        ...
                        <AccordionCard title="4. HARTA BERGERAK" open={openHartaSections.bergerak} onToggle={() => toggleHartaSection('bergerak')} subtitle={formatRupiah(formHartaBergerak)}>
                          <DataGridCard title="Daftar Harta Bergerak" toolbar footerText={`JUMLAH TABEL 4 ${formHartaBergerak.toLocaleString('id-ID')}`}>
                            <div className="p-3 bg-white max-w-md border-b"><FieldInput label="Nilai Harta Bergerak" type="number" value={formHartaBergerak} onChange={setFormHartaBergerak} /></div>
                        ...
                        <AccordionCard title="5. HARTA TIDAK BERGERAK (TERMASUK TANAH BANGUNAN)" open={openHartaSections.tidakBergerak} onToggle={() => toggleHartaSection('tidakBergerak')} subtitle={formatRupiah(formHartaTidakBergerak)}>
                          <DataGridCard title="Daftar Harta Tidak Bergerak" toolbar footerText={`JUMLAH TABEL 5 ${formHartaTidakBergerak.toLocaleString('id-ID')}`}>
                            <div className="p-3 bg-white max-w-md border-b"><FieldInput label="Nilai Harta Tidak Bergerak" type="number" value={formHartaTidakBergerak} onChange={setFormHartaTidakBergerak} /></div>
                        ...
                        <AccordionCard title="6. HARTA LAINNYA" open={openHartaSections.lainnya} onToggle={() => toggleHartaSection('lainnya')}>
                          <DataGridCard title="Daftar Harta Lainnya" toolbar footerText="JUMLAH TABEL 6 0">
                        ...
                        <AccordionCard title="7. IKHTISAR HARTA" open={openHartaSections.ikhtisar} onToggle={() => toggleHartaSection('ikhtisar')} subtitle={formatRupiah(totalHarta)}>
                          <DataGridCard title="Ikhtisar Harta" tightToolbar footerText={`JUMLAH HARTA PADA AKHIR TAHUN PAJAK ${totalHarta.toLocaleString('id-ID')}`}>
                    ...
                    <AccordionCard title="B. UTANG PADA AKHIR TAHUN PAJAK" open={openOpSections.B} onToggle={() => toggleOpSection('B')} subtitle={answerUtang === 'Ya' ? 'Aktif dari jawaban Induk' : 'Tidak aktif'}>
                      <div className="p-4 space-y-3">
                        {answerUtang !== 'Ya' && <InlineNotice tone="warn">Di Induk jawaban utang masih “Tidak”. Klik Tambah Utang untuk mengaktifkan jawaban dan menambahkan data manual.</InlineNotice>}
                        <DataGridCard title="Daftar Utang" toolbar onAdd={handleAddUtangManual} footerText={`JUMLAH BAGIAN B ${formUtang.toLocaleString('id-ID')}`}>
                          <YellowTable columns={['TINDAKAN', 'NO.', 'KODE', 'DESKRIPSI', 'KREDITUR NOMOR IDENTITAS WP', 'KREDITUR NAMA', 'NEGARA KREDITUR', 'TAHUN PINJAMAN', 'SALDO', 'KETERANGAN']}>
                            <FilterRow columns={['TINDAKAN', 'NO.', 'KODE', 'DESKRIPSI', 'KREDITUR NOMOR IDENTITAS WP', 'KREDITUR NAMA', 'NEGARA KREDITUR', 'TAHUN PINJAMAN', 'SALDO', 'KETERANGAN']} />
                            {manualUtangList.length > 0 || formUtang > 0 ? (
                              (manualUtangList.length > 0 ? manualUtangList : [{ kreditur: 'Kreditur Simulasi', tahun: '2025', saldo: formUtang, keterangan: 'Dilaporkan' }]).map((item, index) => (
                                <tr key={`utang-${index}`}>
                                  <td className="border p-2 text-center"><input type="checkbox" /></td>
                                  ...
                                  <td className="border p-2">-</td>
                                  <td className="border p-2">{item.kreditur}</td>
                                  <td className="border p-2">Indonesia</td>
                                  <td className="border p-2 text-center">{item.tahun}</td>
                                  <td className="border p-2 text-right">{onlyNumber(item.saldo).toLocaleString('id-ID')}</td>
                                  <td className="border p-2">{item.keterangan}</td>
                                </tr>
                              ))
                            ) : <EmptyTableBody colSpan={10} text="Tidak ada data untuk ditampilkan." />}
                          </YellowTable>
                        </DataGridCard>
                      </div>
                    </AccordionCard>

                    <AccordionCard title="C. DAFTAR ANGGOTA KELUARGA YANG MENJADI TANGGUNGAN" open={openOpSections.C} onToggle={() => toggleOpSection('C')} subtitle={`${manualKeluargaList.length + clampTanggungan(formTanggungan)} data`}>
                      <div className="p-4 space-y-3">
                        <div className="flex flex-wrap gap-2">
                          <button onClick={() => handleAddKeluargaManual('Anak')} className="h-9 px-4 rounded bg-[#17245c] text-white font-bold flex items-center gap-2"><Plus size={14} /> Tambah Tanggungan</button>
                          <button onClick={() => setCurrentScreen('portal-saya')} className="h-9 px-4 rounded bg-white border text-[#17245c] font-bold flex items-center gap-2"><User size={14} /> Ajukan Gabung NPWP Pasangan</button>
                        </div>
                        <DataGridCard title="Daftar Keluarga" tightToolbar>
                          <YellowTable columns={['NO.', 'NAMA', 'NIK', 'TANGGAL LAHIR', 'HUBUNGAN DENGAN WAJIB PAJAK', 'PEKERJAAN']}>
                            ...
                            {manualKeluargaList.length > 0 || formTanggungan > 0 ? (
                              [
                                ...manualKeluargaList,
                                ...Array.from({ length: Math.max(0, clampTanggungan(formTanggungan) - manualKeluargaList.filter((x) => x.hubungan !== 'Istri/Suami').length) }).map((_, index) => ({ nama: `Tanggungan ${index + 1}`, nik: '-', hubungan: 'Anak', pekerjaan: 'Pelajar' })),
                              ].map((item, index) => (
                                <tr key={`kel-${index}`}>
                                  <td className="border p-2 text-center">{index + 1}</td>
                                  ...
                                  <td className="border p-2">-</td>
                                  <td className="border p-2">{item.hubungan}</td>
                                  <td className="border p-2">{item.pekerjaan}</td>
                                </tr>
                              ))
                    ...
                    <AccordionCard title="D. PENGHASILAN NETO DALAM NEGERI DARI PEKERJAAN" open={openOpSections.D} onToggle={() => toggleOpSection('D')} subtitle={answerPekerjaan === 'Ya' ? formatRupiah(formGaji) : 'Tidak aktif'}>
                      <div className="p-4 space-y-3">
                        {answerPekerjaan !== 'Ya' && <InlineNotice tone="warn">Jawaban Induk untuk penghasilan pekerjaan masih “Tidak”. Klik Tambah Penghasilan untuk menambahkan pemberi kerja manual.</InlineNotice>}
                        <DataGridCard title="Daftar Pemberi Kerja" toolbar onAdd={handleAddPekerjaanManual} footerText={`JUMLAH BAGIAN D ${formGaji.toLocaleString('id-ID')}`}>
                          <YellowTable columns={['TINDAKAN', 'NO.', 'NAMA PEMBERI KERJA', 'NOMOR IDENTITAS PEMBERI KERJA', 'PENGHASILAN BRUTO', 'PENGURANG PENGHASILAN BRUTO/BIAYA', 'PENGHASILAN NETO']}>
                            ...
                            {formGaji > 0 || manualPekerjaanList.length > 0 ? (
                              [
                                ...(formGaji > 0 ? [{ pemberiKerja: 'PT Pemberi Kerja Simulasi', npwp: '01.234.567.8-999.000', bruto: Math.max(0, formGaji - manualPekerjaanList.reduce((sum, item) => sum + onlyNumber(item.neto), 0)), pengurang: 0, neto: Math.max(0, formGaji - manualPekerjaanList.reduce((sum, item) => sum + onlyNumber(item.neto), 0)) }] : []),
                                ...manualPekerjaanList,
                              ].map((item, index) => (
                                <tr key={`kerja-${index}`}>
                                  <td className="border p-2 text-center"><input type="checkbox" /></td>
                                  <td className="border p-2 text-center">{index + 1}</td>
                                  <td className="border p-2">{item.pemberiKerja}</td>
                                  <td className="border p-2">{item.npwp}</td>
                                  <td className="border p-2 text-right">{onlyNumber(item.bruto).toLocaleString('id-ID')}</td>
                                  <td className="border p-2 text-right">{onlyNumber(item.pengurang).toLocaleString('id-ID')}</td>
                                  <td className="border p-2 text-right">{onlyNumber(item.neto).toLocaleString('id-ID')}</td>
                                </tr>
                              ))
                    ...
                    <AccordionCard title="E. DAFTAR BUKTI PEMOTONGAN/PEMUNGUTAN PPh" open={openOpSections.E} onToggle={() => toggleOpSection('E')} subtitle={answerPotongPihakLain === 'Ya' ? formatRupiah(formKreditPajak) : 'Tidak aktif'} badge={activeCase?.kunciManualBupot ? 'kasus manual' : null}>
                      <div className="p-4 space-y-3">
                        {activeCase?.kunciManualBupot && manualBupotList.length === 0 && <InlineNotice tone="warn">Kasus ini meminta bukti potong tambahan yang belum ter-posting otomatis. Klik Tambah Bukti Potong Manual agar validasi bisa lolos.</InlineNotice>}
                        <DataGridCard title="Daftar Bukti Potong/Pungut" toolbar onAdd={handleAddBupotManual} footerText={`JUMLAH BAGIAN E ${formKreditPajak.toLocaleString('id-ID')}`}>
                          <YellowTable columns={['TINDAKAN', 'NO.', 'NAMA PEMOTONG/PEMUNGUT PPh', 'NPWP PEMOTONG/PEMUNGUT PPh', 'NOMOR BUKTI POTONGAN/PEMUNGUTAN', 'TANGGAL BUKTI PEMOTONGAN/PEMUNGUTAN', 'JENIS PAJAK', 'PENGHASILAN BRUTO', 'PPh YANG DIPOTONG']}>
                            <FilterRow columns={['TINDAKAN', 'NO.', 'NAMA PEMOTONG/PEMUNGUT PPh', 'NPWP PEMOTONG/PEMUNGUT PPh', 'NOMOR BUKTI POTONGAN/PEMUNGUTAN', 'TANGGAL BUKTI PEMOTONGAN/PEMUNGUTAN', 'JENIS PAJAK', 'PENGHASILAN BRUTO', 'PPh YANG DIPOTONG']} />
                            {formKreditPajak > 0 || manualBupotList.length > 0 ? (
                              [
                                ...(formKreditPajak > 0 ? [{ pemotong: 'PT Pemberi Kerja Simulasi', npwp: '01.234.567.8-999.000', nomor: 'BP-2025-0001', tanggal: '31-12-2025', jenis: 'PPh 21', bruto: formGaji, pph: Math.max(0, formKreditPajak - manualBupotList.reduce((sum, item) => sum + onlyNumber(item.pph), 0)) }] : []),
                                ...manualBupotList,
                              ].filter((item) => onlyNumber(item.pph) > 0 || item.nomor?.includes('MANUAL')).map((item, index) => (
                                <tr key={`bupot-${index}`}>
                                  <td className="border p-2 text-center"><input type="checkbox" /></td>
                                  <td className="border p-2 text-center">{index + 1}</td>
                                  <td className="border p-2">{item.pemotong}</td>
                                  <td className="border p-2">{item.npwp}</td>
                                  <td className="border p-2">{item.nomor}</td>
                                  <td className="border p-2">{item.tanggal}</td>
                                  <td className="border p-2">{item.jenis}</td>
                                  <td className="border p-2 text-right">{onlyNumber(item.bruto).toLocaleString('id-ID')}</td>
                                  <td className="border p-2 text-right">{onlyNumber(item.pph).toLocaleString('id-ID')}</td>
                                </tr>
                              ))
                            ) : <EmptyTableBody colSpan={9} />}
                          </YellowTable>
                    ...
                    <div className="flex flex-wrap gap-3 pt-2">
                      <button onClick={() => setActiveTab('Induk')} className="h-10 px-5 rounded bg-white border text-[#17245c] font-bold flex items-center gap-2"><ArrowLeft size={15} />Kembali ke Induk</button>
                      <button onClick={handleSimpanKonsep} className="h-10 px-5 rounded bg-[#17245c] text-white font-bold flex items-center gap-2"><Save size={15} />Simpan Konsep</button>
                      <button onClick={handleBatalKonsep} className="h-10 px-5 rounded bg-white border border-rose-200 text-rose-600 font-bold flex items-center gap-2"><X size={15} />Batalkan</button>
                      <button onClick={handleBayarDanLapor} className="h-10 px-5 rounded bg-[#17245c] text-white font-bold flex items-center gap-2"><CreditCard size={15} />Bayar dan Lapor</button>
                    </div>
            ...
            <div className="p-5 space-y-4 text-[12px]">
              <div className="rounded-xl border bg-slate-50 p-4">
                ...
                <div className="mt-2 grid grid-cols-2 gap-3">
                  ...
                  <div className="rounded bg-white border p-3"><div className="text-slate-400">NPWP/NIK</div><div className="font-mono font-bold">{displayTaxpayer?.npwp}</div></div>
        ...
        <div className="fixed inset-0 bg-slate-950/60 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl overflow-hidden">
            <div className="bg-[#17245c] text-white p-4 font-bold flex justify-between items-center">
              Review Ringkasan SPT Sebelum Bayar dan Lapor
              <button onClick={() => setShowReviewModal(false)}><X size={18} /></button>
            </div>
            <div className="p-5 space-y-5 text-[12px] max-h-[82vh] overflow-y-auto">
              <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-amber-900 flex gap-3">
                <Info size={18} className="mt-0.5 flex-shrink-0" />
                <div>
                  <div className="font-black">Review akhir sebelum penyampaian SPT</div>
                  <p className="mt-1 leading-relaxed">
                    Cek kembali identitas, jenis SPT, pajak terutang, kredit pajak, dan status akhir. Setelah klik lanjut, sistem akan meminta tanda tangan digital simulasi.
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                <div className="rounded-xl border bg-slate-50 p-3">
                  <div className="text-slate-400">Jenis Praktikum</div>
                  <div className="font-black text-[#17245c] mt-1">{isBadanSpt ? 'SPT Badan' : 'SPT OP'}</div>
                </div>
                <div className="rounded-xl border bg-slate-50 p-3">
                  <div className="text-slate-400">Status Lifecycle</div>
                  <div className="font-black text-[#17245c] mt-1">{lifecycleMeta.label}</div>
                </div>
                <div className="rounded-xl border bg-slate-50 p-3">
                  <div className="text-slate-400">Pajak Terutang</div>
                  <div className="font-black text-[#17245c] mt-1">{formatRupiah(currentTaxDue)}</div>
                </div>
                <div className={`rounded-xl border p-3 ${currentTaxResult.status === 'KURANG BAYAR' ? 'bg-amber-50 border-amber-200' : currentTaxResult.status === 'LEBIH BAYAR' ? 'bg-rose-50 border-rose-200' : 'bg-emerald-50 border-emerald-200'}`}>
                  <div className="text-slate-500">Status Akhir</div>
                  <div className="font-black mt-1">{currentTaxResult.status}</div>
                  <div className="font-black text-lg">{formatRupiah(currentTaxResult.nominal)}</div>
                </div>
              </div>

              <div className="grid grid-cols-1 lg:grid-cols-[1.35fr_0.65fr] gap-4">
                <div className="rounded-xl border overflow-hidden">
                  <div className="bg-[#ffd542] text-slate-900 font-black px-4 py-3">Detail Ringkasan SPT</div>
                  <div className="divide-y">
                    {getReviewRows().map(([label, value]) => (
                      <div key={label} className="grid grid-cols-[220px_1fr] gap-3 px-4 py-2.5">
                ...
                <div className="rounded-xl border overflow-hidden">
                  <div className="bg-[#17245c] text-white font-black px-4 py-3">Checklist Validasi</div>
                  <div className="p-4 space-y-3">
                    {getValidationChecklist().map((item) => (
                      <div key={item.label} className="flex items-start gap-2">
                        <span className={`mt-0.5 h-5 w-5 rounded-full flex items-center justify-center ${item.done ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}>
                          {item.done ? <Check size={13} /> : <AlertTriangle size={13} />}
                        </span>
                        <span className={item.done ? 'text-slate-700' : 'text-amber-800 font-semibold'}>{item.label}</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div className="flex flex-col sm:flex-row justify-between gap-2 pt-2 border-t">
                <button onClick={handleDownloadRingkasan} className="h-10 px-5 rounded bg-slate-100 text-slate-700 font-bold flex items-center justify-center gap-2 hover:bg-slate-200">
                  <Download size={15} /> Unduh Draft Ringkasan
                </button>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
            ...
            <div className="p-5 space-y-4">
              ...
              <div className="space-y-2 text-[12px]">
                ...
                <div className="flex justify-between border-b pb-2">
                  <span>Nama WP</span>
                  <span className="font-bold">{currentTaxpayerName}</span>
                </div>
              </div>
              <button
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
            ...
            <div className="p-6 text-center space-y-4">
              ...
              <div className="rounded-xl border bg-slate-50 p-4 text-left text-[12px] space-y-2">
                ...
                <div className="flex justify-between border-b pb-2">
                  <span>Jenis SPT</span>
                  <span className="font-bold text-right">{isBadanSpt ? 'SPT Badan' : 'SPT OP'}</span>
                </div>
                <div className="flex justify-between border-b pb-2">
                  <span>NPWP/NIK</span>
                  <span className="font-mono font-bold text-right">{currentTaxpayerId}</span>
                </div>
                <div className="flex justify-between border-b pb-2 gap-4">
                  <span>Nama WP</span>
                  <span className="font-bold text-right">{currentTaxpayerName}</span>
                </div>
                <div className="flex justify-between border-b pb-2">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                <button
                  onClick={handleDownloadBPE}
                  className="h-10 rounded bg-emerald-600 text-white font-bold flex items-center justify-center gap-2 hover:bg-emerald-700"
                >
                  <Download size={15} /> Unduh BPE
                </button>
                <button
                  onClick={handleDownloadRingkasan}
                  className="h-10 rounded bg-slate-100 text-slate-700 font-bold flex items-center justify-center gap-2 hover:bg-slate-200"
                >
                  <FileDown size={15} /> Unduh Ringkasan SPT
                </button>
              </div>
